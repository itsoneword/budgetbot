...
Decisions: `asyncio.to_thread` + module-level render RLock (no process pool). Approved: delete the copy-pasted `show_log_chart` body inside `delete_records` — confirmed during implementation to be LIVE code firing admin chart renders on every /delete (bug, not cruft). Agg pinned before pyplot import; all 6 render functions serialized (RLock — pie chart calls comparison chart internally); the two monthly renders stay sequential awaits (shared DataFrame mutated in place); `convert_dataframe` vectorized with per-currency Decimal factors (verified equivalent to 1e-12 vs row-wise); dead `float_rates` local removed.
