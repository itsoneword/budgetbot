...
### Critical
- [ ] With ADMIN_USER_ID=46304833 in .env, /debug from the admin account toggles debug mode (replies "Debug mode is now ON/OFF")
- [ ] /show_log_chart from the admin account sends the two usage charts
- [ ] /debug from a non-admin account replies "Sorry, only admin users can toggle debug mode."
- [ ] /show_log_chart from a non-admin account replies "This command is restricted to the bot owner."
...
- [ ] Container picks up the env var: `docker compose exec budgetbot python -c "from src.config import ADMIN_USER_ID; print(ADMIN_USER_ID)"` prints 46304833
- [ ] Bot stays responsive (returns to normal transaction handling) after a denied admin command
